...
function serveStatic(app, relativeUrl, dirPath, config, configGlobal) {
	...
	log.info(`Serve static files: ${configGlobal.hostname}:${config.port}${relativeUrl} => ${staticPath}`)
}
...
function startServer(port, initFuncs, config, configGlobal) {
	...
	for (const initFunc of initFuncs) {
		initFunc(app, config, configGlobal)
	}
			...
			const initFuncs = item
				...
				.map(init => {
					...
					if (typeof init !== 'function') {
						...
						init = (app, cfg, cfgGlobal) => serveStatic(app, relativeUrl, dirPath, cfg, cfgGlobal)
					}
